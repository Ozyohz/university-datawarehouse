"""
Bronze Layer - Enrollment Data Ingestion
=========================================
Ingests enrollment data from source systems into the bronze layer.
"""
...
def create_spark_session():
    ...
    return SparkSession.builder \
        .appName("Bronze - Ingest Enrollments") \
...
def get_source_schema():
    """Define the expected schema for enrollment data"""
    return StructType([
        ...
        StructField("midterm_score", DecimalType(5, 2), True),
        StructField("final_score", DecimalType(5, 2), True),
        StructField("total_score", DecimalType(5, 2), True),
        StructField("grade_letter", StringType(), True),
        StructField("status", StringType(), True),
        StructField("attendance_count", IntegerType(), True),
        StructField("absence_count", IntegerType(), True),
    ])
...
def main():
    ...
    # MinIO/S3 paths
    source_path = os.getenv("SOURCE_PATH", "s3a://raw-data/enrollments/")
    bronze_path = os.getenv("BRONZE_PATH", "s3a://bronze/enrollments/")
    ...
    try:
        print(f"Starting enrollment ingestion - Batch ID: {batch_id}")
        
        # Read from source
        df = ingest_from_files(spark, source_path, file_format="csv")
        
        # Add metadata columns and cast dates for Delta Lake too
        ...
        print("Writing to PostgreSQL bronze.raw_enrollments")
        write_to_postgres(
            df_with_metadata, 
            jdbc_url, 
        ...
        print(f"Successfully ingested {record_count} enrollment records")
