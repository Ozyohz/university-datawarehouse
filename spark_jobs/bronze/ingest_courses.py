"""
Bronze Layer - Course Data Ingestion
======================================
Ingests course data from source systems into the bronze layer.
"""
...
def get_source_schema():
    """Define the expected schema for course data"""
    return StructType([
        StructField("course_id", StringType(), True),
        StructField("course_name", StringType(), True),
        StructField("course_description", StringType(), True),
        StructField("credits", IntegerType(), True),
        StructField("department_id", StringType(), True),
        StructField("course_level", StringType(), True),
...
def main():
    ...
    # MinIO/S3 paths
    source_path = os.getenv("SOURCE_PATH", "s3a://raw-data/courses/")
    bronze_path = os.getenv("BRONZE_PATH", "s3a://bronze/courses/")
    ...
    try:
        print(f"Starting course ingestion - Batch ID: {batch_id}")
        ...
        print("Writing to PostgreSQL bronze.raw_courses")
        write_to_postgres(
            df_with_metadata, 
            jdbc_url, 
            "bronze.raw_courses", 
        ...
        print(f"Successfully ingested {record_count} course records")
