"""
Silver Layer - Department Data Transformation
==============================================
Cleanses and transforms department data from bronze to silver layer.
"""
...
def create_spark_session():
    ...
    return SparkSession.builder \
        .appName("Silver - Transform Departments") \
...
def clean_string_columns(df):
    ...
    return df \
        ...
        .withColumn("faculty", trim(col("faculty"))) \
        .withColumn("dean_name", trim(col("dean_name")))
...
def validate_data(df):
    ...
    return df.withColumn(
        ...
        "validation_errors",
        when(col("department_id").isNull(), array(lit("Missing department_id")))
        .when(col("department_name").isNull(), array(lit("Missing department_name")))
        .otherwise(array())
    )
...
def write_to_postgres(df, jdbc_url, table_name, properties, mode="overwrite"):
    ...
    silver_columns = [
        "department_id", "department_name", "faculty", "dean_name", 
        "established_year", "is_valid", "validation_errors"
    ]
...
def main():
    ...
    try:
        print("Starting department transformation: Bronze → Silver")
        
        # Read from bronze
        df_bronze = read_from_postgres(spark, jdbc_url, "bronze.raw_departments", properties)
        ...
        # Deduplicate
        df_deduped = deduplicate(df_transformed, ["department_id"])
        
        # Write to PostgreSQL
        print("Writing to PostgreSQL silver.stg_departments")
        write_to_postgres(df_deduped, jdbc_url, "silver.stg_departments", properties)
        
        print("Department transformation completed successfully")
